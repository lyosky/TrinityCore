...
public:
    ...
    void OnUpdateZone(Player* player, uint32 /*newZone*/, uint32 /*oldZone*/, uint32 /*newArea*/)
    {
        HandlePhase(player);
    }
...
#define GOSSIP_MASTER_MATHIAS_SHAW "跟我说说雄狮之眠发生了什么。"
    ...
    void sGossipHello(Player* player)
    {
        ...
        {//player->GetQuestStatus(QUEST_SUMMONS_TO_STORMWIND)==QUEST_STATUS_REWARDED &&
         ...
         //SendGossipMenuFor(player, player->GetGossipTextId(creature), creature->GetGUID());
            ClearGossipMenuFor(player);
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, string_To_UTF8(GOSSIP_MASTER_MATHIAS_SHAW), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
            SendGossipMenuFor(player, 33712, me->GetGUID());
        }
        else
        {
...
#define GOSSIP_ZIDORMI_2 "你能带我返回现在吗？"
struct npc_zidormi_128607 : public ScriptedAI
{
    ...
    void sGossipHello(Player* player)
    {
        ...
        if (player->GetMapId() == 1)
        {
            ...
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, string_To_UTF8(GOSSIP_ZIDORMI_2), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            SendGossipMenuFor(player, 33094, me->GetGUID());
        }
        else
        {
            ///GossipID: 21720 TextID: 33093 GossipOptions: 1  GossipText: 0
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, string_To_UTF8(GOSSIP_ZIDORMI_1), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
...
struct npc_nolan_speed_131963 : public ScriptedAI
{
    ...
    enum Texts
    {
        SAY_FIRST_LINE = 0, ///我们收到的命令是，用一切必要的手段获取这种神秘的矿物。
        SAY_SECOND_LINE = 1,///暮光之锤再一次崛起的话，我们承受不了。虚无行者必须死！
        SAY_THIRD_LINE = 2 ///我想希利苏斯的情况已经在我们的掌控之中。接下来的行动就由军情七处继续进行吧。再次感谢你，$n。
    };
...
struct npc_kelsey_steelspark_130030 : public ScriptedAI
{
    ...
    enum Texts
    {
        SAY_FIRST_LINE = 0, ///这些贪得无厌的地精！真想看看他们发现切割机被炸成碎片时脸上的表情！ 
    };
...
struct npc_pixni_rustbomb_132040 : public ScriptedAI
{
    ...
    enum Texts
    {
        SAY_FIRST_LINE = 0, ///别怕弄脏你的手，把这些异种蝎的头敲碎吧！
        SAY_SECOND_LINE = 1,///这项研究可以帮助我们了解那把巨剑的力量！
    };
...
///132147 QUEST_H_THE_BLOOD_OF_AZEROTH
struct npc_grol_warblade_132147 : public ScriptedAI
{
    npc_grol_warblade_132147(Creature* creature) : ScriptedAI(creature) { }
    enum Texts
    {
        SAY_FIRST_LINE = 0, ///去那边看看，让那些懒惰的勘探者知道自己的份量！
        SAY_SECOND_LINE = 1,///我们不能让这个疯子把幸存者集结起来！干掉他，把他的徽记给我带来！
        SAY_THIRD_LINE = 2, ///希利苏斯的情况已经处于掌控之中
    };
...
///-- PHASE 10401
#define GOSSIP_NATHANOS_BLIGHTCALLER "跟我说说宴会上发生了什么。"
